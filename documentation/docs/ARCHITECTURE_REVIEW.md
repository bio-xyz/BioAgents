...
3. **Queue mode and in-process mode still diverge semantically.** Chat now dispatches through `runChatAgent` in both transports, but deep research keeps separate in-process and worker implementations. **Impact**: behavior can differ between local/dev and production depending on `USE_JOB_QUEUE`. **Recommendation**: converge on one execution core per capability and use adapters for sync/async transport only.
