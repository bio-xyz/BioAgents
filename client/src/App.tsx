...
export function App() {
  ...
  const isCurrentConversationLoading =
    isLoading && loadingConversationId === currentSessionId;
  ...
  useEffect(() => {
    ...
    const { finalResponse, steps, isDeepResearch, messageId } =
      currentState.values;
    ...
    if (finalResponse && steps) {
      const allStepsComplete = Object.values(steps).every(
        (step: any) => step.end,
      );
  ...
  useEffect(() => {
    ...
    const messagesNeedingStates = assistantMessages.filter(
      (m) => !m.thinkingState,
    );
    ...
    console.log(
      "[App] Fetching states for",
      messagesNeedingStates.length,
      "messages",
    );
  ...
  const handleSend = async (mode: string = "normal") => {
    ...
    console.log(
      "[App] Adding user message immediately, current messages.length:",
      messages.length,
    );
      ...
      if (mode === "deep") {
        ...
        if (response.status === "rejected") {
          ...
          addMessage({
            ...
            content:
              response.error ||
              "Deep research request was rejected. Please check the format.",
          });
          scrollToBottom();
        ...
        <div className="main-content">
              ...
              >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <p
                      style={{ margin: 0, fontSize: "13px", color: "#a1a1a1" }}
                    >
          ...
          {x402Enabled && walletAddress && hasInsufficientBalance && (
            ...
            >
              ...
              >
                You need at least $0.10 USDC to make payments. Your current
                balance is ${usdcBalance || "0.00"} USDC.
              </span>
              <div
                style={{
                  fontSize: "0.85rem",
