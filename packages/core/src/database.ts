...
export abstract class DatabaseAdapter<DB = unknown> implements IDatabaseAdapter {
  ...
  abstract addNewInternalKnowledgeEntry(
    wallet: string,
    hasInternalKnowledge: boolean
  ): Promise<void>;
  abstract hasInternalKnowledge(wallet: string): Promise<boolean>;

  // Answer evaluation operations
  abstract createAnswerEval(data: {
    messageId?: string;
    channelId: string;
    agentId: string;
    senderId: string;
  ...
  abstract updateAnswerEval(messageId: string, updates: {
    answer?: string;
    knowledgeChunks?: any[];
    knowledgeGraphChunks?: any[];
    knowledgeGraphSynthesis?: string;
