...
export class AgentRuntime implements IAgentRuntime {
  ...
  /**
   * Broadcasts a message state update via the internal event system
   * @param channelId - The channel/room ID where the processing is happening
   * @param state - The current processing state
   * @param messageId - Optional message ID being processed
   */
  async broadcastMessageState(
    channelId: UUID,
    state: MESSAGE_STATE,
    messageId?: UUID
  ): Promise<void> {
    try {
      const stateUpdate: MessageStateUpdate = {
        type: 'messageState',
        state,
        roomId: channelId,
        messageId,
      ...
      // Emit the state update event which will be caught by the server's socket infrastructure
      await this.emitEvent('MESSAGE_STATE_UPDATE', stateUpdate);

      this.logger.info(`[Runtime] Broadcasted ${state} state for channel ${channelId}`);
    } catch (error) {
      this.logger.warn(`[Runtime] Failed to broadcast message state:`, error);
    }
  }
  ...
  async addNewInternalKnowledgeEntry(wallet: string, hasInternalKnowledge: boolean): Promise<void> {
    return await this.adapter.addNewInternalKnowledgeEntry(wallet, hasInternalKnowledge);
  }

  async hasInternalKnowledge(wallet: string): Promise<boolean> {
    return await this.adapter.hasInternalKnowledge(wallet);
  }
  ...
  async createAnswerEval(data: {
    ...
    return await this.adapter.createAnswerEval(data);
  }

  async updateAnswerEval(messageId: string, updates: {
    answer?: string;
    knowledgeChunks?: any[];
    knowledgeGraphChunks?: any[];
    knowledgeGraphSynthesis?: string;
    ...
    return await this.adapter.updateAnswerEval(messageId, updates);
  }
}
