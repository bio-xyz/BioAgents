...
export function setupSocketIO(
  ...
  io.use(async (socket, next) => {
    try {
      ...
      // Check for server-to-server with X-API-KEY
      const serverAuthToken = process.env.ELIZA_SERVER_AUTH_TOKEN;
      const apiKey = socket.handshake.headers['x-api-key'];

      if (serverAuthToken && apiKey === serverAuthToken) {
        logger.info('Socket.IO: Valid X-API-KEY provided, bypassing JWT auth');
        return next();
      }
      ...
      if (process.env.NODE_ENV === 'staging') {
        ...
        if (origin === 'http://164.92.185.3') {
          return next();
        }
      }

      // Check for JWT token in auth or headers
      const token =
      ...
      logger.info('Socket.IO: JWT token verified successfully');
      next();
    } catch (error) {
  ...
  // Setup message state broadcasting from runtime events
  // We need to register this event listener with each agent's runtime
  // This will be done when agents are registered
  logger.info('Socket.IO setup complete with message state broadcasting support');
