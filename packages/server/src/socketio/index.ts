...
export class SocketIORouter {
  ...
  private async handleMessageSubmission(socket: Socket, payload: any) {
    const useInternalKnowledge = payload.useInternalKnowledge || false;
    const channelId = payload.channelId || payload.roomId; // Support both for backward compatibility
    const { senderId, senderName, message, serverId, source, metadata, attachments } = payload;

    logger.info(
      `[SocketIO ${socket.id}] Received SEND_MESSAGE for central submission: channel ${channelId} from ${senderName || senderId}, useInternalKnowledge: ${useInternalKnowledge}`
    );
    logger.info(
    ...
    try {
      ...
      if (isDmForWorldSetup && senderId) {
        ...
        const runtime = Array.from(this.agents.values())[0];
        if (runtime) {
          // for private chats we consider using internal knowledge
          if (source === 'client_chat' && useInternalKnowledge) {
            runtime.addNewInternalKnowledgeEntry(senderId, useInternalKnowledge);
          }
          runtime.emitEvent(EventType.ENTITY_JOINED as any, {
