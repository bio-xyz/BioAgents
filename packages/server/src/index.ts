...
export class AgentServer {
  ...
  private async initializeServer(options?: ServerOptions) {
    try {
      ...
      // Enable trust proxy for rate limiting and security when behind reverse proxy
  ...
  public async registerAgent(runtime: IAgentRuntime) {
    try {
      ...
      // Register MESSAGE_STATE_UPDATE event handler for socket broadcasting
      if (this.socketIO) {
        runtime.registerEvent('MESSAGE_STATE_UPDATE', async (stateUpdate: MessageStateUpdate) => {
          try {
            this.socketIO!.to(stateUpdate.roomId).emit('messageState', stateUpdate);
            logger.info(
              `Message state broadcasted: ${stateUpdate.state} for room ${stateUpdate.roomId}`
            );
          } catch (error) {
            logger.warn('Failed to broadcast message state via socket:', error);
          }
        });
      }
