...
export class KnowledgeService extends Service {
  ...
  async addKnowledge(options: AddKnowledgeOptions): Promise<{
    ...
    try {
      ...
      if (existingDocument && existingDocument.metadata?.type === MemoryType.DOCUMENT) {
        ...
        const chunksDbName = options.chunksDbName || 'knowledge';
        const fragments = await this.runtime.getMemories({
          tableName: chunksDbName,
        });
  ...
  private async processDocument({
    ...
    try {
      ...
      const fragmentCount = await processFragmentsSynchronously({
        ...
        chunksDbName: chunksDbName || 'knowledge',
      });
  ...
  async getKnowledge(
    message: Memory,
    scope?: { roomId?: UUID; worldId?: UUID; entityId?: UUID; chunksDbName?: string }
    ...
    const chunksDbName = scope?.chunksDbName || 'knowledge';
  ...
  async _internalAddKnowledge(
    ...
    for (const fragment of fragments) {
      try {
        await this.processDocumentFragment(fragment, chunksDbName); // fragment already has metadata.documentId from splitAndCreateFragments
        fragmentsProcessed++;
      } catch (error) {
        logger.error(
  ...
  private async processDocumentFragment(
    fragment: Memory,
    chunksDbName = 'knowledge'
  ): Promise<void> {
    try {
      ...
      await this.runtime.createMemory(fragment, chunksDbName);
    } catch (error) {
      logger.error(
