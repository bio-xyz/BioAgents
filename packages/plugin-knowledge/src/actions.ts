...
export const searchKnowledgeAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      console.log('Using internal knowledge in knowledge plugin action:', useInternalKnowledge);
      ...
      const knowledgeResults = await service.getKnowledge(searchMessage, {
      ...
      if (useInternalKnowledge) {
        // Search user knowledge (can also be dynamic, e.g. based on user's wallet)
        userKnowledgeResults = await service.getKnowledge(searchMessage, {
      ...
      let allResults: KnowledgeItem[] = [];
      if (userKnowledgeResults.length) {
        allResults = [...knowledgeResults.slice(0, 3), ...userKnowledgeResults.slice(0, 2)];
      } else {
        allResults = knowledgeResults;
      }

      if (allResults.length === 0) {
        response = {
          text: `I couldn't find any information about "${query}" in my knowledge bases.`,
        };
      } else {
        // Format results
        const formattedResults = allResults
          .map((item, index) => {
            return `${index + 1}. ${item.content.text}`;
          })
          .join('\n\n');
