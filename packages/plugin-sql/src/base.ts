...
export abstract class BaseDrizzleAdapter extends DatabaseAdapter<any> {
  ...
  /**
   * Adds a new internal knowledge entry for a user wallet
   */
  async addNewInternalKnowledgeEntry(wallet: string, hasInternalKnowledge: boolean): Promise<void> {
    return this.withDatabase(async () => {
      const now = new Date();
      await this.db
        .insert(internalKnowledgeTable)
        .values({
          wallet,
        ...
        .onConflictDoUpdate({
          target: internalKnowledgeTable.wallet,
          set: {
            hasInternalKnowledge,
            updatedAt: now,
          },
  ...
  /**
   * Checks if a wallet has internal knowledge
   * @param wallet The wallet address to check
   * @returns True if the wallet has internal knowledge, false otherwise
   */
  async hasInternalKnowledge(wallet: string): Promise<boolean> {
    return this.withDatabase(async () => {
      const result = await this.db
        .select()
        .from(internalKnowledgeTable)
        .where(eq(internalKnowledgeTable.wallet, wallet))
        .limit(1);

      return result.length > 0 && result[0].hasInternalKnowledge;
    });
  }

  /**
   * Creates a new answer evaluation record
   */
  async createAnswerEval(data: {
    messageId?: string;
    channelId: string;
    agentId: string;
    senderId: string;
    ...
    answer?: string;
    knowledgeChunks?: any[];
    knowledgeGraphChunks?: any[];
    knowledgeGraphSynthesis?: string;
    ...
    return this.withDatabase(async () => {
      ...
      logger.info('Creating answer eval with data: ', data.messageId);
      ...
      const evalToInsert = {
        ...
        knowledgeGraphChunks: data.knowledgeGraphChunks || [],
        knowledgeGraphSynthesis: data.knowledgeGraphSynthesis,
        responseTimeMs: data.responseTimeMs,
        createdAt: now,
        updatedAt: now,
      };

      const result = await this.db.insert(answerEvalTable).values(evalToInsert).returning();
      ...
      return {
        ...
        knowledgeChunks: result[0].knowledgeChunks as any[],
        knowledgeGraphChunks: result[0].knowledgeGraphChunks as any[],
        knowledgeGraphSynthesis: result[0].knowledgeGraphSynthesis,
        responseTimeMs: result[0].responseTimeMs,
  ...
  /**
   * Updates an existing answer evaluation record by messageId
   */
  async updateAnswerEval(
    messageId: string,
    updates: {
      answer?: string;
      knowledgeChunks?: any[];
    ...
    return this.withDatabase(async () => {
      ...
      logger.info(
        'Updating answer eval with id and field names: ',
        messageId,
        Object.keys(updates)
      );
      ...
      if (updates.answer !== undefined) {
        updateValues.answer = updates.answer;
      }
      if (updates.knowledgeChunks !== undefined) {
        updateValues.knowledgeChunks = sql`${JSON.stringify(updates.knowledgeChunks)}::jsonb`;
      }
      if (updates.knowledgeGraphChunks !== undefined) {
        updateValues.knowledgeGraphChunks = sql`${JSON.stringify(updates.knowledgeGraphChunks)}::jsonb`;
      }
      if (updates.knowledgeGraphSynthesis !== undefined) {
        updateValues.knowledgeGraphSynthesis = updates.knowledgeGraphSynthesis;
      }
      if (updates.responseTimeMs !== undefined) {
        updateValues.responseTimeMs = updates.responseTimeMs;
      }

      const result = await this.db
        .update(answerEvalTable)
        .set(updateValues)
        .where(eq(answerEvalTable.messageId, messageId));
