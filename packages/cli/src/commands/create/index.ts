...
import { createProject, createPlugin, createAgent } from './actions';
import type { CreateOptions } from './types';

export const create = new Command('create')
  .description('Create a new ElizaOS project, plugin, or agent')
  ...
  .option('--type <type>', 'type of project to create (project, plugin, agent)', 'project')
    ...
    try {
      ...
      if (!projectName) {
        if (!isNonInteractive) {
          ...
          projectType = selectedType as 'project' | 'plugin' | 'agent';
        }
