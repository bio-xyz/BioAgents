...
import { SOCKET_MESSAGE_TYPE, type MessageStateUpdate } from '@elizaos/core';
import { Evt } from 'evt';
...
// Define type for message state updates
export type MessageStateData = MessageStateUpdate;
...
export class SocketIOManager extends EventAdapter {
  ...
  public get evtMessageState() {
    return this._getEvt('messageState') as Evt<MessageStateData>;
  }
  ...
  public initialize(clientEntityId: string): void {
    ...
    this.socket.on('messageState', (data: MessageStateData) => {
      clientLogger.info(`[SocketIO] Message state update received:`, data);

      // Check if this is for one of our active channels
      const channelId = data.roomId;
      if (channelId && this.activeChannelIds.has(channelId)) {
        clientLogger.info(
          `[SocketIO] Handling message state update for active channel ${channelId}: ${data.state}`
        );

        // Emit the message state event
        ...
        // For debugging: emit anyway to see if it helps
        clientLogger.info(`[SocketIO] DEBUG: Emitting message state anyway for debugging`);
        this.emit('messageState', data);
      }
    });
  }
