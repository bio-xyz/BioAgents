...
/**
 * Parse CSV content for message attachment analysis
 * Creates a concise summary for agent conversation context
 * @param csvContent Raw CSV content
 * @param filename Original filename
 * @returns Formatted text summary of CSV data
 */
function parseCSVForAttachment(csvContent: string, filename: string): string {
  try {
    // Split into lines and handle different line endings
    const lines = csvContent.split(/\r\n|\r|\n/).filter((line) => line.trim() !== '');

    if (lines.length === 0) {
      return `Empty CSV file: ${filename}`;
    }
    ...
    // Process up to 10,000 rows for comprehensive analysis
    const maxRows = Math.min(10000, totalRows);
    const processedRows: string[][] = [];
    let summary = `CSV File: ${filename}\n`;
    summary += `CSV Data with ${totalRows} rows and ${headers.length} columns\n\n`;
    summary += `Column Headers: ${headers.join(', ')}\n\n`;

    // Process all rows (up to 10k)
    for (let i = 0; i < maxRows; i++) {
      ...
      // Show first 20 rows in detail, then just collect for stats
      if (i < 20) {
        summary += `Row ${i + 1}:\n`;
    ...
    if (totalRows > 20) {
      summary += `\n[Showing first 20 of ${totalRows} rows in detail. All ${Math.min(totalRows, 10000)} rows processed for statistics.]\n`;
    }
    if (totalRows > 10000) {
      summary += `\n[Note: CSV contains ${totalRows} total rows. Only first 10,000 rows processed.]\n`;
    }

    // Add statistical summary for numeric columns
    const numericStats = calculateCSVStatistics(processedRows, headers);
    if (numericStats.length > 0) {
      summary += '\nColumn Statistics:\n';
      numericStats.forEach((stat) => {
        summary += `  ${stat.column}: ${stat.summary}\n`;
      });
    }

    return summary;
  } catch (error: any) {
    return `Error parsing CSV file ${filename}: ${error.message}`;
  }
}
 ...
 */
function calculateCSVStatistics(
  dataRows: string[][],
  headers: string[]
): Array<{ column: string; summary: string }> {
  ...
  for (const attachment of attachments) {
    try {
      ...
      if (attachment.contentType === ContentType.IMAGE && !attachment.description) {
        ...
        !attachment.text &&
        ((attachment.contentType as string) === ContentType.DOCUMENT ||
          (attachment.contentType as string) === 'text/csv' ||
          (attachment.contentType as string) === 'text/plain' ||
          attachment.url?.toLowerCase().endsWith('.csv') ||
          attachment.url?.toLowerCase().endsWith('.txt'))
      ) {
        const res = await fetch(url);
        ...
        const isCSV = contentType === 'text/csv' || attachment.url?.toLowerCase().endsWith('.csv');
        ...
        if (isPlainText) {
          ...
          logger.debug(`[Bootstrap] Processing CSV document: ${attachment.url}`);

          const csvContent = await res.text();
          const parsedCSV = parseCSVForAttachment(csvContent, attachment.url || 'data.csv');
          processedAttachment.text = parsedCSV;
          processedAttachment.title = processedAttachment.title || 'CSV Data';

          logger.debug(
            `[Bootstrap] Extracted CSV summary (first 200 chars): ${processedAttachment.text?.substring(0, 200)}...`
          );
        } else {
          logger.warn(`[Bootstrap] Skipping non-text/csv document: ${contentType}`);
        }
      }
  ...
  try {
    ...
    const processingPromise = (async () => {
      try {
        ...
        if (shouldRespond) {
          const room = await runtime.getRoom(message.roomId);
          // Broadcast THINKING state when agent decides to respond
          await runtime.broadcastMessageState(
            room?.channelId as UUID,
            MESSAGE_STATE.THINKING,
            message.id
          );
          ...
          if (message.id && room?.channelId && message.content?.source !== 'twitter') {
            // Create answer eval for the message
            await runtime.createAnswerEval({
              messageId: message.id,
          ...
          while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
            // Use the smaller model for the message handler
