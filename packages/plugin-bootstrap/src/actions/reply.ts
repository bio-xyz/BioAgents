...
export const replyAction = {
  ...
  handler: async (
    ...
    // Broadcast REPLYING state when starting to generate response
    await runtime.broadcastMessageState(
      room?.channelId as UUID,
      MESSAGE_STATE.REPLYING,
      message.id
    );
    ...
    // if source is twitter. use twitter prompt
    if (message.content.source === 'twitter') {
      prompt = composePromptFromState({
        state,
        template: runtime.character.templates?.twitterReplyTemplate ?? '',
      });
    } else {
      prompt = composePromptFromState({
        state,
    ...
    if (message.createdAt) {
      await runtime.updateAnswerEval(message.id as string, {
        responseTimeMs: Date.now() - message.createdAt,
        answer: response.message as string,
      });
    }

    await callback(responseContent);

    // Broadcast DONE state after response is sent
    await runtime.broadcastMessageState(room?.channelId as UUID, MESSAGE_STATE.DONE, message.id);
